...
\usepackage{iftex}
\ifPDFTeX
    \usepackage[utf8]{inputenc} % utf-8 standard zur direkten Einfabe von Umlauten
    \usepackage[T1]{fontenc} % Font endcoding auf T1: 8-Bit encoding zur Darstellung von Umlauten in der Ausgabe
%    \usepackage{lmodern} % Latin modern wird in Fonts.tex gesetzt
\else
    \ifXeTeX
        \usepackage{xltxtra}
    \else
...
%\usepackage{tabls} %Reserviert Abstände in Tabellen
...
%   % PDF Informationen
%   pdftitle={},             % Titel
   pdfauthor={Tammo Schwindt},            % Autor
...
\usepackage{makeidx}		% Index -->Vor Hyperref
\IfDraft{
...
\usepackage[
%    xindy, % Nutzung von xindy an Stelle von makeindex (default) zum indizieren (empfohlen, da xindy auch non-english sprachen unterstützt)
...
%   textwidth=17cm,    % modifies \textwidth, the width of body
%   textheight=25cm,   % modifies \textheight, the height of body
   %body=,        % { width , height } sets both \textwidth and \textheight of the body of page.
   %lines=30,       % enables users to specify \textheight by the number of lines.
...
%%% Margin
   left=2.5cm,        % left margin (for oneside) or inner margin (for twoside) of total body
                  % alias: lmargin, inner
   right=2.5cm,       % right or outer margin of total body
                  % alias: rmargin outer
   top=2.5cm,         % top margin of the page.
                  % Alias : tmargin
   bottom=2.5cm,      % bottom margin of the page
                  % Alias : bmargin
%   hmargin=2.5cm,     % left and right margin. hmargin={ left margin , right margin }
...
%%% Dimensionen
   %headheight=1cm,  % Alias:  head
   %headsep=0.5cm,     % separation between header and text
   %footskip=1cm,    % distance separation between baseline of last line of text and baseline of footer
                  % Alias: foot
...
%    \ofoot[\pagemark]{\pagemark}
   ...
   \ohead{\pagemark}
   \ihead{\headmark}
%   \ohead[\pagemark]{\pagemark} % [ Außen unten: Seitenzahlen bei plain
}{
...
\setkomafont{pagehead}{\small\rmfamily}        % Kopfzeile
...
% Epigraph zum Erstellen von Zitaten mit dem Befehl \epigraph{<Zitat>}{<Author>}
\usepackage{epigraph} % Fuer zentrierte Zitate
\epigraphsize{\normalsize}
\setlength\epigraphwidth{0.8\textwidth}
\setlength\epigraphrule{0pt}

% Mit diesem paket koennen Bilder in dem includegraphics makro ueber eine
% zusaetzliche Option rechts, links oder zentriert dargestellt werden
% Optionen: right, left, center
% \includegraphics[right]{image}
...
%\usepackage{booktabs}    %
%\usepackage{sidecap}    %
%\usepackage{wrapfig}    % Einbinden von Bildern in den Text: Text umfließt das Bild (Bereits eingebunden)
%\usepackage{setspace}    % individuelles Setzen von Zeilenabständen 
\usepackage{here}    % Zum Festsetzen von Figure Positionen
%\usepackage{hyperref}    % Notwendig für die Verlinkung von Seiten im PDF (vor allem im TOC) (Bereits eingebunden)
%\usepackage{verbatim} %%Verwenden des \begin{comment} und \end{comment} (Bereits eingebunden)
%\usepackage{longtable} %%Zum Darstellen grosser Tabellen
%\usepackage{lscape} %%Um Seiten im Querformat darstellen zu können   
%
% - - - - - - - - - - - - - - - - - - - - - 